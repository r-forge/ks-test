...
several variants on a procedure proposed by
...
\citep{stephens1974}.  In the early 1970's, 
...
The test statistic is, in terms of the theory, the same as in the
...
&\sup_x \left| f(x)- g(x) \right|  \nonumber  \\
       &= \max_i \left( \max\left( \left|g(x_i) - f(x_i) \right|, \lim_{x \rightarrow x_i} \left| g(x) - f(x_{i-1}) \right| \right)\right) \\
&=  \max_i \bigg(\max \bigg( \left|g(x_i) - f(x_i) \right|, \left| g(x_i) - f(x_{i-1}) \right| \bigg) \bigg).
\end{align*}
...
with the standard null distribution. 
...
The relative powers of these tests to different alternatives are
studied in depth in \cite{stephens1974}. The $W^2$
...
The null distribution of the test statistics are again hypothesis-dependent. The methods do not offer finite sample results, but rather 
show that the asymptotic distribution of the test statistic under the null hypothesis is equal to the distribution of a weighted sum of independent
...
%*** What does it mean for a distribution to be equal to a weighted sum of
...
formulation); it should not greatly affect the interpretation of the results.

%*** How is the cutoff established in the code? ***

Figure \ref{cvmissues} shows the non-monotonicity of the numerical
evaluation of equation 7 for the null distribution which is
uniform on the set $\{1,2,3\}$, as well as the behavior of the
conservative approximation in equation 9.
...
(pink) versus a conservative chi-squared bound (dashed blue). The null distribution
is unform on the set $\{1,2,3\}$. }
\label{cvmissues}
\end{figure}
