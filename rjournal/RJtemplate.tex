...
The function \pkg{cvm.test()} is similar in design
to \pkg{ks.test()}.  Its first two
...
\code{htest}.  Although we designed \code{cmv.test()} primarily for cases with
discrete distributions \code{y}, we implemented the continuous forms of the
test for variants \code{W2} and \code{A2} (as noted in the Discussion, below).
...
This paper presents the implementation of a range of nonparametric
...
In particular, functions implemented in package \pkg{nortest} focus on the
composite hypothesis of normality, while
package \pkg{ADGofTest} provides the Anderson-Darling variant
of the test for continuous distributions.  Packages
\pkg{CvM2SL1Test} and \pkg{CvM2SL1Test} provide support for
two-sample Cram\'{e}r-von Mises tests with continuous distributions.
Package \pkg{cramer} offers a multivariate Cram\'{e}r test for
 ...
 *** Need to be added
to the bibliography, whatever we choose to do ***
